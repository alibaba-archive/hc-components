...
import Mocky from 'hc-mocker/mocky';
...
let acNames = [];
let Antd = {};
export default class VizWrapper extends React.PureComponent {
  static contextTypes = {
  ...
  static propTypes = {
    ...
    mocker: PropTypes.object
  };

  static getRoutes = (components) => {
    Antd = components;
    acNames = Object.keys(Antd);
    ...
    acNames.forEach(name => {
      ...
      if (Com.prototype && Com.prototype.isReactComponent) {
        ...
        class Scene extends React.Component {
          ...
          render() {
            return <VizWrapper name={name} />;
          }
        }
        childRoutes.push({
          path: 'antd/' + name,
          resolvePath: 'antd/' + name,
          name: name,
          title: name,
          navKey: 'antd',
          component: Scene,
  ...
  constructor(props, context) {
    ...
    this._mocker = props.mocker || mocker;
    ...
    if (this._propTypesSchema) {
      this.state = this.format(
        this.dataSource,
        this._propTypesSchema.properties
      );
    } else {
      this.state = {};
    }
  ...
  parseJsx(jsx, name) {
    ...
    if (jsx.match(childrenReg)) {
      ...
      this._jsx = jsx.replace(
        new RegExp('<' + tagName + '\\s*([\\s\\S]*?)>([\\s\\S]*?)</\\1>'),
        ($0, $1, $2, $3, $4, $5) => {
          const reg = /([\w\-_]*)=[{"]([^"}]*)["}]/g;
  ...
  runCode() {
    const code = convertor.parse(this.getCode().replace(/\/\/[^;]+;\n*/g, ''));
    this.setState({
      children: code
    });
  }
  ...
  handleChange(v, noForce) {
    if (!Antd[v]) {
      ...
      this._propTypesSchema =
        antdPropTypes[v + 'Props'] || parserProps(this._component);
    } else {
      this._component = Antd[v];
    ...
    if (
      ...
      !['checked', 'message', 'defaultValue', 'value', 'fileList'].filter(
        k => !!this._propTypesSchema.properties[k]
      ).length
    ) {
      this._propTypesSchema.properties.children = {
        type: 'string',
              ...
              this.setHtml(
                ...
                () => {
                  try {
                    ...
                    const getCode = this._vizWrapper.getCode.bind(
                      this._vizWrapper
                    );
                    Object.assign(this._childOption, {
                      getCode: () => {
                        ...
                        if (this._childOption.jsx) {
                          return this._childOption.jsx.replace(
                            childrenReg,
                            code
                          );
                        } else {
          ...
          <Popover
            ...
            content={
              <Input.TextArea
                ...
                ref={inst => (this._previewInput = inst)}
                disabled={true}
              />
            }
            title="预览引用代码"
                ...
                () => {
                  if (this._input) {
                    if (this._input.textAreaRef.value.match(childrenReg)) {
                      ...
                      this._childOption.code = this._childOption.jsx.replace(
                        childrenReg,
  ...
  componentDidMount() {
    ...
    if (this.refs.trigger1) {
      this._clipboard1 = new Clipboard(
        ReactDOM.findDOMNode(this.refs.trigger1),
        {
          text: () => {
            return convertor.transform(this._code) || '';
          }
        }
      );
      this._clipboard1.on('success', e => {
        message.info('复制成功');
        e.clearSelection();
      });
    }
    if (this.refs.trigger2) {
      this._clipboard2 = new Clipboard(
        ReactDOM.findDOMNode(this.refs.trigger2),
        {
          text: () => {
            return this._code;
          }
  ...
  componentWillUnmount() {
    this._clipboard1 && this._clipboard1.destroy();
    this._clipboard2 && this._clipboard2.destroy();
    this.modal_ && this.modal_.destroy();
  }

  setHtml(title, callback, extra) {
    this.modal_ = Modal.confirm(
      Object.assign(
        {
          title: title,
          iconType: 'setting',
          ...
          content: (
            <Input.TextArea
              defaultValue={extra && extra.value}
              ref={inst => (this._input = inst)}
              autosize={{
                minRows: 6,
                maxRows: 12
              }}
          ...
          >
            ...
            >
              ...
              <Button
                type="default"
                onClick={() => {
                  this.propTypeForm.handleSubmit(body => {
                    const required = [];
                    for (let name in body.propTypes) {
                      if (body.propTypes[name] !== undefined) {
                        this._propTypesSchema.properties[name].default =
                          body.propTypes[name];
                        required.push(name);
                      }
                    }
                    this._propTypesSchema._required =
                      this._propTypesSchema.required || [];
              ...
              <Button
                ...
                onClick={() => {
                  if (this._propTypesSchema._required) {
                    this._propTypesSchema.required.forEach(name => {
              ...
              {this._temple ? (
                <Dropdown.Button
                  ...
                  overlay={
                    ...
                    >
                      {this._temple.list.map((item, index) => (
              ...
              <Button type="dashed">
                <a
                  href={`https://ant.design/components/${snakeCase(
                    this.name
                  ).replace(/_/g, '-') + '-cn'}/#API`}
                  target="_blank"
                >
            ...
            {this.props.routes ? null : (
              ...
              >
                <Button type="default" ref="trigger1">
                  复制执行代码
                </Button>
                <Button
                  type="default"
                  ref="trigger2"
                  onMouseDown={() => (this._code = this.getCode())}
                >
                  复制源码
                </Button>
              </Button.Group>
            ...
            <DataSet
              formatter={formatter.form.options}
              data={{
                options: {
